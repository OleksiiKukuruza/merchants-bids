...
const withLifecycle = lifecycle({
  componentDidMount() {
    const { fetchMerchantById, match: { params: { id } } } = this.props;
...
const withForm = reduxForm({
  form: 'editMerchant',
  onSubmit: (merchant, dispatch, { updateMerchant }) => updateMerchant(merchant),
  onSubmitSuccess: (merchant, dispatch, { history }) => history.push('/merchants')
});
